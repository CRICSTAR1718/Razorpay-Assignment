...
export default function CFODashboard() {
    ...
    async function fetchPending() {
        ...
        try {
            ...
            // Spec: pending CFO action (best-effort status mapping)
            setPending(
                list.filter(
                    (r) => r.status === 'PENDING' || r.status === 'CFO_PENDING' || r.status === 'CFO_REVIEW_PENDING'
                )
            );
        } catch (e) {
                    ...
                    <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <p className="text-sm text-gray-600">Waiting for CFO approval.</p>
                            </div>
                            <button
                                type="button"
